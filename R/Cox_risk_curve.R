#' @title Individual Dynamic Risk Curve for Cox-PACE Model
#' @description Generates dynamic conditional survival probability curves for individual patients
...
#' @param model Fitted Cox-PACE model object from Cox_pace function
...
#'
#' @return A recorded plot object displaying the dynamic conditional survival probability curve
#'
...
#' print(risk_plot)
#' }
Cox_risk_curve <- function(i_data,model,pred_time,fixed_cov,tv_cov,tv_cov_regular,tv_continuous_cov,L,w,rtime,id){
...
#################
  ...
  cCox<-data.frame(time=pred_time,surv=NA)

  for (i.p5 in 1:length(pred_time)) {

    out<-summary(survfit(model$model,newdata=dat_com[i.p5,]))
    tmp<-evalstep(out$time,out$surv,c(pred_time[i.p5],pred_time[i.p5]+w),subst=1)
    cCox[i.p5,2]<-tmp[2]/tmp[1]
  }

  par(xaxs="i", yaxs="i", mar=c(5,5,3,2))
  y_range <- range(cCox[,2], na.rm = TRUE)
  y_padding <- diff(y_range) * 0.2
  ylim_range <- c(y_range[1] - y_padding, y_range[2] + y_padding)
  plot(cCox[,1], cCox[,2], type="l", lwd=6, lty=1, bty="l", xlab="", ylab="", col=4, las = 1,ylim = ylim_range)
  title(main=list(paste0("Patient ID: ", data_pre_1$id[1], " (Cox)"), font=7, cex=1.5), line=1)
  title(xlab="Prediction Time", font.lab=7, cex.lab=1.5, line=2.6)
  title(ylab="Conditional  Survival  Probability", font.lab=7, cex.lab=1.3, line=2.9)
