#' @title Dynamic RMST Prediction Model With PACE Method
#' @description Implements a dynamic prediction framework that combines landmark RMST models
...
#'     when building dynamic RMST models.
#'
...
#' \itemize{
#'   \item \code{model}: Fitted Dynamic RMST model
...
RMST_pace<-function(data,LMs,w,L,id,time,rtime,status,fixed_cov=c(),tv_cov=c(),tv_cov_regular=c(),tv_continuous_cov=c(),formula_cov,mea_error=0){
  ...
  for (k1 in seq(along=LMs)){
    ...
    LM$pseudo=pseudomean(time=LM$time-LMs[k1],event=LM$status,tmax=w)
    ALM<-rbind(ALM,LM)
  }
  ...
  while(nrow(ALM)!=nrow(na.omit(ALM)) ){
    for (j8 in 1:length(tv_cov)){ #j8=1
  ...
  D.RMST<-geeglm(as.formula(paste0("pseudo ~ ", formula_cov, " + LM1 + LM2")),
                 data=G.ALM, id=id, scale.fix=FALSE, family=gaussian, corstr="independence")

  return(list(model=D.RMST,PACE.fit=PACE.fit))
}
